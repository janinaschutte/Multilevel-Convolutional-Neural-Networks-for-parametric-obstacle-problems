...
class Problem(ParallelizableProblem):
    def __init__(self, info):
        assert info['problem']['name'] == "obstacle"
        self.info = info
        ...
        if info['expansion']['type'] == "karhunen_loeve":
            ...
            if self.obstacle_param:
                self.field = TestField(M-1, mean=mean, scale=scale, decay=decay, expfield=expfield)
            else:
    ...
    def plain_solution(self, y, level, verbose=False):
        ...
        # build operator
        if self.obstacle_param:
            obstacle_backend = y[-1].item()
            y = y[:-1]
        else:
            obstacle_backend = -0.035
